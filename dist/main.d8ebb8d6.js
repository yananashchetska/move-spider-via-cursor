...
wall.addEventListener('click', function (ev) {
  ...
  var maximalX = wall.clientWidth - spider.clientWidth;
  var maximalY = wall.clientHeight - spider.clientHeight;

  // на скільки далеко має розміщуватись центр павука

  var centerX = ev.offsetX - spider.clientWidth / 2;
  var centerY = ev.offsetY - spider.clientHeight / 2;
  ...
  var spiderX = Math.max(0, Math.min(maximalX, centerX));
  var spiderY = Math.max(0, Math.min(maximalY, centerY));
